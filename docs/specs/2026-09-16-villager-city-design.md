...
- Detects vanilla villages: a bell (`meeting` POI) with at least one villager linked to it
  within 64 blocks. Scans when a chunk containing a bell POI loads.
...
- `villageId`, `job: JobType` (`NONE`, `LUMBERJACK`, `BUILDER`), `taskState` (serialized).
...
- Each tick for an employed citizen: if vanilla priority activities are active (`PANIC`, `REST`,
  `RAID`, `HIDE`, or the villager is trading with a player), our task pauses and keeps its state.
  Otherwise the current `Task` ticks and the vanilla `WORK`/`IDLE` activities are suppressed.
...
- Slice 1 places it automatically at the first valid plot next to the bell when a village
  registers.
...
**`Blueprint`**
- Loaded from `data/villagercity/blueprints/*.nbt` (vanilla `StructureTemplate` format).
...
- Slice 1 ships one blueprint: a small oak-plank house with a bed, authored with a structure block.

**`PlotPlanner` (slice 1 rule)**
- Candidate positions spiral outward from the bell. A plot is valid if the blueprint footprint
  plus a 1-block margin is inside village bounds + 32, the ground height varies by at most 1,
  there are no fluids, and it does not overlap an existing building record or plot.
